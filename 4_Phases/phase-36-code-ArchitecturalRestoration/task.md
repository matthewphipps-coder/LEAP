# GoldSource Phase: 36 - ArchitecturalRestoration
...
- **FOLDER JAIL**: ALL artifacts MUST be in `4_Phases/phase-36-code-ArchitecturalRestoration/`.
...
- **Prototype**: "N/A"
- **Workflow**: direct
- **Discovery Artifact**: `4_Phases/phase-36-code-ArchitecturalRestoration/stage-1-discovery.yaml`
- **Backup Location**: `X_GoldSource_Backup/phase-36-{timestamp}`
...
```bash
rm -rf 2_GoldSource
cp -r X_GoldSource_Backup/phase-36-{timestamp} 2_GoldSource
```
...
- [x] Create phase folder: `mkdir -p 4_Phases/phase-36-code-ArchitecturalRestoration`
...
```bash
cp -r 2_GoldSource X_GoldSource_Backup/phase-36-$(date +%Y%m%d-%H%M)
```

- [x] **Create timestamped backup of GoldSource FIRST**
- [x] Modify scaffold files per design
- [x] WIP commit: `git commit -m 'WIP Phase 36: [description]'`
...
- [x] Run Governance Check: `bash 1_LEAP/1_Governance/3_resources/gov-check.sh` (MUST PASS)
...
- [x] Capture screenshots to `4_Phases/phase-36-code-ArchitecturalRestoration/`
...
- [ ] Final commit: `git commit -m 'Phase 36: [phase name] complete'`
