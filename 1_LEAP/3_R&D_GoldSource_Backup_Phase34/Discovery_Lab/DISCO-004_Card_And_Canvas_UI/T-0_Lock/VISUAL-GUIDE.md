...
/* Free Form Mode - Spatial Layout */
.nexus-canvas.freeform-mode {
  display: block; /* No grid constraint */
  position: relative;
  min-height: 800px; /* Space for dragging */
}

.nexus-canvas.freeform-mode .nexus-card {
  position: absolute; /* XY positioning */
  margin: 0;
  z-index: 1; /* Default layer */
  width: 300px; /* Standardize width for free movement */
}
